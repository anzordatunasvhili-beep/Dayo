...
create table public.profiles (
  ...
  role public.user_role not null default 'student', email text,
  first_name text not null, last_name text not null,
  teacher_id uuid references public.profiles(id) on delete cascade,
  studio_code text, country_region text, created_at timestamptz not null default now(),
...
begin
  ...
  values (new.id, coalesce((new.raw_user_meta_data->>'role')::public.user_role, 'teacher'), new.email,
    coalesce(new.raw_user_meta_data->>'first_name','Teacher'), coalesce(new.raw_user_meta_data->>'last_name','Account'),
    nullif(new.raw_user_meta_data->>'teacher_id','')::uuid, new.raw_user_meta_data->>'studio_code', new.raw_user_meta_data->>'country_region')
